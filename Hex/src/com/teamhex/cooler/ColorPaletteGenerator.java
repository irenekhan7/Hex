...
public class ColorPaletteGenerator 
{
	...
	public static int[] colorAlgorithm(int[] pixels, int k)
	{
		...
		if (k < n)
		{
			...
			ArrayList<Integer>[] sets = (ArrayList<Integer>[]) new ArrayList[k];
			
			for(int a = 0; a < k; a++)
				sets[a] = new ArrayList<Integer>();
			
			HashMap<Integer, Integer> pairings = new HashMap<Integer, Integer>();
			while (true)
			{
				int updates = 0;
				for (int i = 0; i < n; i++)
				{
					...
					if (pairings.get(i) == null || pairings.get(i) != min)
					{
						updates++;
